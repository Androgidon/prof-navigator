...
from app.domains.question_bank.selection_service import QuestionSelectionService
...
class AssessmentSessionService:
    ...
    async def start(self, assessment_slug: str, user_id: Optional[str] = None):
        catalog = await self.catalog_repository.get_by_slug(assessment_slug)
        if not catalog:
            return None, None
        question_ids = await self.selection_service.select_for_assessment(assessment_slug)
