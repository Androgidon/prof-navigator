    ...
    <div className="results-page">
      ...
      <main className="results-content">
        ...
        <div className="results-header">
          ...
          <p className="results-subheading">Express показывает профиль и направления, Full — точный Top-8 профессий и персональные рекомендации.</p>
        </div>
          ...
          return (
            <>
                  ...
                  <article key={`${direction.title}-${direction.rank}`} className="recommendation-card">
                    ...
                    <div className="card-tags">
                      {direction.example_professions.slice(0, 5).map((item) => (
                        <span key={`${item.profession_slug}-${item.title}`} className="tag-chip tag-subject">{item.title}</span>
                      ))}
                    </div>
                  </article>
                ))}
              ...
              <div className="settings-card" style={{ marginTop: "1rem" }}>
                <h3 className="dashboard-section-heading" style={{ marginBottom: "0.5rem" }}>{express.monetization_cta.title}</h3>
                ...
                <Link
                  ...
                  onClick={() => {
                    void emitCtaClicked(express.result_id, "express", express.monetization_cta.target_action, express.monetization_cta.target_url);
                  }}
                >
          ...
          return (
            <>
              {recommendations.length > 0 && (
                <ExplanationPanel title="Краткий разбор результата" description="Сильные стороны и уверенность по результатам завершенного прохождения.">
